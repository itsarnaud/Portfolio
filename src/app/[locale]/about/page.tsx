...
import { skills, getValues } from '@/src/lib/data';
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-start">
          ...
          <div className="lg:pt-12">
            ...
            <div className="flex flex-wrap gap-6 mt-12">
              <MagneticButton>
                <Link href="/contact" className="inline-flex items-center gap-2 px-8 py-4 bg-foreground text-background text-sm tracking-wide hover:opacity-90 transition-opacity">
                  {t('contactBtn')}
                </Link>
              </MagneticButton>
              <MagneticButton>
                <a href="/files/cv.pdf" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-8 py-4 border border-foreground text-sm tracking-wide hover:bg-foreground hover:text-background transition-colors">
                  {t('cvBtn')}
      ...
      <section className="px-6 md:px-12 lg:px-24 mb-32 py-24 bg-secondary/50">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid lg:grid-cols-2 gap-16">
            <div>
              <h3 className="text-lg font-medium mb-8">{t('frontend')}</h3>
            ...
            <div>
              <h3 className="text-lg font-medium mb-8">{t('backend')}</h3>
              <div className="space-y-6">
          ...
          <div className="mt-16">
            <h3 className="text-lg font-medium mb-8">{t('tools')}</h3>
            <div className="flex flex-wrap gap-3">
              {skills.tools.map((tool) => (
                <span key={tool} className="px-4 py-2 border border-border text-sm hover:border-foreground transition-colors">{tool}</span>
              ))}
            </div>
          </div>
          <div className="mt-16">
            <h3 className="text-lg font-medium mb-8">{t('learning')}</h3>
            <div className="flex flex-wrap gap-3">
              {skills.learning.map((tech) => (
                <span key={tech} className="px-4 py-2 border border-dashed border-border text-sm text-muted-foreground hover:border-foreground hover:text-foreground transition-colors">{tech}</span>
              ))}
            </div>
          </div>
        </div>

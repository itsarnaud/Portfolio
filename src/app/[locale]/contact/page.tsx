...
const ContactPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const subjectLabels: Record<string, string> = {
      project:       t('subjectProject'),
      collaboration: t('subjectCollaboration'),
      job:           t('subjectJob'),
            ...
            {isSubmitted ? (
              <div className="p-8 border border-foreground bg-secondary/50">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <span className="text-lg font-medium">{t('successTitle')}</span>
                </div>
              ...
              <form ref={formRef} onSubmit={handleSubmit} className="space-y-8">
                ...
                <div className="form-field">
                  <label htmlFor="name" className={`block text-sm mb-2 transition-colors ${focusedField === "name" ? "text-foreground" : "text-muted-foreground"}`}>
                    {t('nameLabel')}
                  </label>
                  <input
                    type="text" id="name" name="name" value={formState.name} onChange={handleChange}
                ...
                <div className="form-field">
                  <label htmlFor="email" className={`block text-sm mb-2 transition-colors ${focusedField === "email" ? "text-foreground" : "text-muted-foreground"}`}>
                    {t('emailLabel')}
                  </label>
                  <input
                    type="email" id="email" name="email" value={formState.email} onChange={handleChange}
                ...
                <div className="form-field">
                  <label htmlFor="subject" className={`block text-sm mb-2 transition-colors ${focusedField === "subject" ? "text-foreground" : "text-muted-foreground"}`}>
                    {t('subjectLabel')}
                  </label>
                  <select
                    id="subject" name="subject" value={formState.subject} onChange={handleChange}
                ...
                <div className="form-field">
                  <label htmlFor="message" className={`block text-sm mb-2 transition-colors ${focusedField === "message" ? "text-foreground" : "text-muted-foreground"}`}>
                    {t('messageLabel')}
                  </label>
                  <textarea
                ...
                <div className="form-field pt-4">
                  <MagneticButton>
                    <button type="submit" disabled={isSubmitting} className="inline-flex items-center gap-3 px-10 py-5 bg-foreground text-background text-sm tracking-wide hover:opacity-90 transition-opacity disabled:opacity-50">
                      {isSubmitting ? (
                        <>
